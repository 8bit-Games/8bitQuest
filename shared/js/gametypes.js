...
Types.isItem = function (kind) {
  return (
    Types.isWeapon(kind) || Types.isArmor(kind) || (Types.isObject(kind) && !Types.isChest(kind))
  );
};
...
Types.isExpendableItem = function (kind) {
  return (
    Types.isHealingItem(kind) || kind === Types.Entities.FIREPOTION || kind === Types.Entities.CAKE
  );
};
